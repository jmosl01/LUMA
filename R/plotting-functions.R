...
EIC.plotter=function(anposGa,Sample.df,Peak.list,center,BLANK,gen.plots,ion.mode,file.base,QC.id) {
  if(missing(BLANK)) BLANK = FALSE
  if(missing(gen.plots)) gen.plots = FALSE
  if(missing(QC.id)) QC.id = "Pooled_QC"
  ...
  validate.df <- Peak.list.pspec[order(Peak.list.pspec$metabolite_group),c("MS.ID","mz","rt","Name","Formula","Annotated.adduct","isotopes","adduct","mono_mass","metabolite_group","Correlation.stat")]
  Peak.list.new <- list(Peak.list.pspec,validate.df)
  return(Peak.list.new) #Use the second element in the list to validate CAMERA
  #! Very important!! Needs a sample index for each new psspectrum; without it, it will only find files
  #! for the first n spectra, where n is the original number of psspectra.
  #! Find out how CAMERA performs automatic selection, then replicate here for the new psspectra
  #! For now, I am using the center sample for every psspectra
  new_anposGa@psSamples <- rep(center, length(X))  #Tells CAMERA to select all psspectra from the center file
  # new_anposGa@psSamples <- rep(-1, length(X))  #Experimental
  # new_anposGa@sample <- as.numeric(NA) #doesn't work for some reason
  ...
  if(gen.plots && !BLANK){
    ...
    sample.cols <- grep(paste(QC.id,paste(strsplit(sexes, "(?<=.[_])", perl = TRUE), collapse = "|"),sep = "|"), colnames(Peak.list))
    ...
    for (i in 1:length(get.mg)) {
      ...
      if(nrow(my.df) == 1) {} else {
        if(nrow(my.df) == 2) {
          my.mat <- as.matrix(my.df[,grep(paste(QC.id,paste(strsplit(sexes, "(?<=.[_])", perl = TRUE), collapse = "|"),sep = "|"),colnames(my.df))])
          ...
          text(x = 0.5, y = 0.5, paste("Dendrograms can't be plotted for two features.\n",
                                       ...
                                       "Tell if two features are from the same metabolite"),
               cex = 1.6, col = "black")
          res2 <- rcorr(test.mat)
          # Insignificant correlations are leaved blank
          corrplot(res2$r, type="upper", order="hclust",
                   p.mat = res2$P, sig.level = 0.01, insig = "blank", tl.col = rainbow(maxlabel))
          ...
          # corrplot(res2$r, type="upper", order="hclust",
          #          p.mat = res2$P, sig.level = 0.01, insig = "pch", pch = 3)
          # Insignificant correlations are leaved blank
          corrplot(res2$r, type="upper", order="hclust",
                   p.mat = res2$P, sig.level = 0.01, insig = "blank", tl.col = rainbow(maxlabel))
          #Plots the EICs and Pseudo-spectra for each metabolite group containing more than one feature in a for loop
...
EIC.dup.plotter=function(anposGa,xpos.cor,annegGa,xneg.cor,Peak.list,gen.plots,file.base,QC.id,...){
  if(missing(Peak.list)) Peak.list = NULL
  if(missing(gen.plots)) gen.plots = FALSE
  if(missing(xpos.cor)) xpos.cor = anposGa@xcmsSet
  if(missing(xneg.cor)) xneg.cor = annegGa@xcmsSet
  if(missing(QC.id)) QC.id = "Pooled_QC"
  if(missing(myname)) myname = NULL
  if(missing(peak.db)) peak.db = NULL
  if(missing(file.base)) file.base = "EIC_plots"

  if(is.null(Peak.list)) {
    if(is.null(myname)||is.null(peak.db)) {stop("Must specify database parameters if not providing Peak.list!", call. = FALSE)}
  }
  ...
  Dup.ID.Pos<-Peak.list$Duplicate_ID[sapply(res, function(x) x == TRUE)&sapply(Peak.list$`Ion Mode`, function(x) x =="Pos")]
  Dup.ID.Neg<-Peak.list$Duplicate_ID[sapply(res, function(x) x == TRUE)&sapply(Peak.list$`Ion Mode`, function(x) x =="Neg")]
  ...
  if(gen.plots) {
    ...
    for (i in Un.ID) {
      ...
      if(length(EIC.list) == 1) {
      ...
      } else if(length(EIC.pos)*length(EIC.neg) != 0){                 ## has duplicate in both modes
        pos=list()
        ...
        rt.min <- min(rt[,"rtmin"])
        rt.max <- max(rt[,"rtmax"])
        color.palette<-c("black", "blue", "mediumslateblue", "red", "green", "khaki3","lavenderblush3", "lawngreen", "lightseagreen", "purple","skyblue1","darkgreen", "darkgoldenrod1", "seashell4", "sienna3", "salmon4", "hotpink")
        ...
        for (j in 1:length(EIC.pos)){
          ...
          title(sub=paste(paste("Positive #",Index.Pos[j],", EIC_ID:", EIC.pos[j]),paste(strwrap(Name.pos[j], width = 0.9*getOption("width")),collapse = "\n"),sep = "\n"),cex.sub = 1, col.sub = "limegreen", line = 6)
        ...
        for (j in 1:length(EIC.neg)){
          ...
          title(sub=paste(paste("Negative #",Index.Neg[j],", EIC_ID:", EIC.neg[j]),paste(strwrap(Name.neg[j], width = 0.9*getOption("width")),collapse = "\n"),sep = "\n"),cex.sub = 1, col.sub = "red", line = 6)
          abline(v=rt, col="blue", lty=2)
        }
      } else if(length(EIC.pos) == 0){                           ## has duplicate in negative mode ONLY
        neg=list()
        for (j in 1:length(EIC.neg)){
      ...
      } else {                                                 ## has duplicate in positive mode ONLY
        pos=list()
        for (j in 1:length(EIC.pos)){
