...
FormatForSIMCA=function(Peak.list=NULL,Sample.df,Sample.data,tbl.id=NULL,QC.id="Pooled_QC_",...) {
  ...
  Sample.data <- setNames(cbind.data.frame(Sample.data[,1], group, Sample.data[-1]), c(colnames(Sample.data[1]),"Exposure Class", colnames(Sample.data[-1])))
  ...
  if(all(is.na(sample.ID))) {  ##Alternatively uses a unique alphanumeric ID provided by user
    sample.ID <- sub('\\D*(\\d{6}).*', '\\1',colnames(sample.peaks))
  }
  sample.peaks <- cbind.data.frame(Peak.list[,"MS.ID"], sample.peaks)
  #Create a new ID for each metabolite and record how many features were combined into that metabolite for metadata
  ...
  #make a dataframe of metabolite intensity values for pooled QC samples then transpose so the colnames are
  #the same as metabolite names: Pos_###_Annotated
  ...
  #make a dataframe of all metadata columns then transpose so the colnames are the same as the
  #metabolite names: Pos_###_Annotated
  log.list <- list()
  log.list[[1]] <- res
  log.list[[2]] <- QC
  ...
  # pheno.peaks <- Peak.list[,-grep("[MF]_",colnames(Peak.list))]
  # colnames(pheno.peaks)
