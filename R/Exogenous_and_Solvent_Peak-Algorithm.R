#' @title FindExogenousandSolventPeak
#'
#' @export
#' @description Searches mz/rt features for exogenous features.  Defined by mean abundance in endogenous samples below user-defined threshold
...
FindExogenousandSolventPeak=function(Peak.list,Sample.df,search.par,ion.id,QC.id,MB.id,tbl.id,db.id,ion.modes,...){
  if(missing(ion.id)) {ion.id = c("Pos","Neg")}
  if(missing(QC.id)) QC.id = "Pooled_QC_"
  if(missing(MB.id)) MB.id = "^MB"
  if(missing(tbl.id)) tbl.id = NULL
  if(missing(db.id)) db.id = c("Peaklist","Blanks")
  if(is.null(Peak.list) && is.null(tbl.id)) {stop("Need to specify tbl.id if using databases to retrieve Peak.list!", call. = FALSE)}
  ...
  endo.groups <- as.matrix(paste(Sample.df[which(Sample.df[,"Endogenous"] == TRUE),"Sex"],Sample.df[which(Sample.df[,"Endogenous"] == TRUE),"Class"],sep = "_"))
  ...
  for(i in 1:list.length) {
    ...
    bin = vector(mode = "logical",
                 length = search.list %>% nrow)
    ...
    for(j in 1:nrow(search.list)) {
      ...
      test.list <- IHL %>%
        ...
        filter(
          between(rt,rt.min,rt.max)) %>%
        dplyr::collect()
      if (nrow(test.list) == 0) {
      } else {
        if(nrow(test.list) == 1 ) {
          if(search.list$sample.mean[j]/test.list$mean <= Solvent.ratio) {
            cat("\n\n\nSolvent peak removed for match number ", counter, " in ",cur.ion," ionization.\n", sep = "")
            temp <- test.list[,c("MS.ID", "Formula", "Name", "Annotated.adduct", "Conf.Level", "FISh.Coverage", "isotopes", "adduct")]
            bin[j] = TRUE
            counter = counter + 1
          }
          ...
          if(nrow(test.list) >= 1) {
            if(search.list$sample.mean[j]/max(test.list$mean) <= Solvent.ratio) {
              cat("\n\n\nSolvent peak removed for match number ", counter, ".\n", sep = "")
              temp <- test.list[,c("MS.ID", "Formula", "Name", "Annotated.adduct", "Conf.Level", "FISh.Coverage", "isotopes", "adduct")]
              bin[j] = TRUE
              counter = counter + 1
            }
...
#' @param ... Arguments to pass parameters to database functions
#' @return NULL testing
PreCombineIonModes=function(Peak.list,tbl.id,...) {
  if(missing(tbl.id)) tbl.id = NULL
  if(is.null(tbl.id) && is.null(Peak.list)) {stop("Need to specify tbl.id if using databases to retrieve Peak.list!", call. = FALSE)}
  if(is.null(Peak.list)) {
    ...
    # Error check
    if(length(grep("Positive",names(Peak.list))) == 0 || length(grep("Negative",names(Peak.list))) == 0) {stop("Peaklist must be a named list. Try \n> names(Peak.list) = c(\"Positive\",\"Negative\")", call. = FALSE)}
